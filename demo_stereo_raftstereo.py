...
def evalute(atom_dict, 
    ...
    viser.save_pred_vis(-flow_pr_sequence[-1].data.numpy()[0], imageGT_file)
...
def validate_eth3d(model, iters=32, root="", sv_root="", mixed_prec=False, args=None):
    ...
    model.eval()
    aug_params = {}
    val_dataset = datasets.ETH3D(aug_params, root=root, args=args)
...
def validate_kitti(model, iters=32, root="", sv_root="", mixed_prec=False, args=None):
    ...
    model.eval()
    aug_params = {}
    val_dataset = datasets.KITTI(aug_params, root=root, image_set='testing', args=args)
...
def validate_things(model, iters=32, root='', sv_root="", mixed_prec=False, args=None):
    ...
    model.eval()
    val_dataset = datasets.SceneFlowDatasets(dstype='frames_finalpass', root=root, things_test=True, eval=True, image_set="test", args=args)
...
def validate_middlebury(model, iters=32, split='F', root="", sv_root="", mixed_prec=False, args=None):
    ...
    model.eval()
    aug_params = {}
    val_dataset = datasets.Middlebury(aug_params, root=root, split=split, image_set="test", args=args)
...
@torch.no_grad()
def validate_booster(model, iters=32, root="", mixed_prec=False, sv_root="", image_set="test", args=None):
    """ Peform validation using the Booster (TRAIN balanced) split """
    model.eval()
    aug_params = {}
    val_dataset = datasets.Booster(aug_params, root=root, image_set=image_set, args=args)
