...
from core.utils.utils import InputPadder, LoggerCommon, load_intermediate_results
...
def evalute(atom_dict, 
            ...
            padder, viser, dataset_name, bad_thold, args=None):
...
def validate_eth3d(model, iters=32, root="", sv_root="", mixed_prec=False, args=None):
    ...
    for val_id in range(len(val_dataset)):
        ...
        image_epe, image_out = evalute(atom_dict, 
                                       image1, image2, flow_gt, valid_gt, imageGT_file, 
                                       padder, viser, dataset_name="ETH3D", bad_thold=1.0, args=args)
...
def validate_kitti(model, iters=32, root="", sv_root="", mixed_prec=False, args=None):
    ...
    for val_id in range(len(val_dataset)):
        ...
        image_epe, image_out = evalute(atom_dict, 
                                       image1, image2, flow_gt, valid_gt, imageGT_file, 
                                       padder, viser, dataset_name="kitti", bad_thold=3.0, args=args)
        epe_list.append(image_epe)
        out_list.append(image_out)
...
def validate_middlebury(model, iters=32, split='F', root="", sv_root="", mixed_prec=False, args=None):
    ...
    for val_id in range(len(val_dataset)):
        ...
        image_epe, image_out = evalute(atom_dict, 
                                       image1, image2, flow_gt, valid_gt, imageGT_file, 
                                       padder, viser, dataset_name="Middlebury", bad_thold=2.0, args=args)
        epe_list.append(image_epe)
        out_list.append(image_out)
...
def validate_booster(model, iters=32, root="", mixed_prec=False, sv_root="", image_set="train/balanced", args=None):
    ...
    for val_id in range(len(val_dataset)):
        ...
        image_epe, image_out = evalute(atom_dict, 
                                       ...
                                       padder, viser, dataset_name="Booster", bad_thold=2.0, args=args)
        epe_list.append(image_epe)
        out_list.append(image_out)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vis_inter', action='store_true', help="visualize intermediate results, like feature map, confidence.")
    ...
    # args.root = "./datasets/Booster"
    # viser = Visualizer(args.root, args.sv_root, "booster", scratch=False, args=args, logger=logger)
    # vis_inter(viser, args)
