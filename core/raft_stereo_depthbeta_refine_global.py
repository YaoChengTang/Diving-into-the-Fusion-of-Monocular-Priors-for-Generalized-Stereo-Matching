...
from core.fusion import BetaModulator, RefinementMonStereoGlobal
...
class RAFTStereoDepthBetaRefineGlobal(nn.Module):
    def __init__(self, args):
        super(RAFTStereoDepthBetaRefineGlobal, self).__init__()
        self.args = args
        ...
        self.refinement = RefinementMonStereoGlobal(args, hidden_dim=args.hidden_dims[-1])
        ...
        if not hasattr(self.args, "fintune_info") or "tune_refine" in self.args.fintune_info.lower().split(" ") :
            ...
            # train raftstereo architecture
            for module in [self.refinement]:
                for param in module.parameters():
                    param.requires_grad = False
        
        elif "tune_register" in self.args.fintune_info.lower().split(" ") :
            # train depth registration part in refinement
            for module in [self.cnet, self.update_block, self.lbp_encoder, 
                           self.modulater, self.context_zqr_convs, self.fnet]:
                for param in module.parameters():
                    param.requires_grad = False
            
            # Also freeze everything in refinement by default
            for param in self.refinement.parameters():
                param.requires_grad = False

            # Unfreeze only mono_params_estimate and shift_refinement
            for param in self.refinement.mono_params_estimate.parameters():
                param.requires_grad = True
            for param in self.refinement.shift_refinement.parameters():
                param.requires_grad = True
        
        elif "tune_register_shift_refine" in self.args.fintune_info.lower().split(" ") :
            # train depth registration part in refinement
            for module in [self.cnet, self.update_block, self.lbp_encoder, 
                           self.modulater, self.context_zqr_convs, self.fnet]:
                for param in module.parameters():
            ...
            # Unfreeze only shift_refinement and mono_params_estimate
            for param in self.refinement.shift_refinement.parameters():
                param.requires_grad = True
    ...
    def forward(self, image1, image2, iters=12, disp_init=None, test_mode=False, vis_mode=False, intrinsic=None):
        ...
        if not hasattr(self.args, "fintune_info") or \
           ...
           "tune_register" in self.args.fintune_info.lower().split(" ") or \
           "tune_register_shift_refine" in self.args.fintune_info.lower().split(" "):
            # refinement
            corr = corr_fn(hor_coords1)
            disp = -hor_coords1 + hor_coords0
            disp_refine, up_mask, depth_registered, conf = self.refinement(disp, depth, net_list[0], corr, distribution, ctx_fea=cnet_list[0][1])
